...
export class MediaLibraryComponent implements OnInit, OnDestroy {
  ...
  @Output() mediaUploaded = new EventEmitter<MediaItem>();
  ...
  loadExistingMedia(): void {
    console.log('[MediaLibraryComponent] loadExistingMedia called');
    this.isLoading = true;
    this.mediaService.getList({ maxResultCount: 100, skipCount: 0, sorting: '' })
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          console.log('[MediaLibraryComponent] getList response', response);
          if (response.items) {
        ...
        error: (error) => {
          console.error('[MediaLibraryComponent] Error loading media:', error);
          this.isLoading = false;
          // Show a user-friendly error message (optional)
          alert('Failed to load media from the server. Please check your backend/API connection.');
        },
        complete: () => {
          this.isLoading = false;
        }
